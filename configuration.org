...
Emacs dependencies/libraries are managed with the straight package manager.
...
#+END_SRC
** Install packages
First tell straight that we should use the built-in version of org-roam
...
#+BEGIN_SRC emacs-lisp
  (straight-use-package '(deflate :type git :host github :repo "skuro/deflate"))
  ...
  (straight-use-package '(yaml-mode :type git :host github :repo "yoshiki/yaml-mode"))
  (straight-use-package '(elixir-ls :type git :host github :repo "elixir-lsp/elixir-ls" :tag "v0.30.0"))
  ;; (straight-use-package '(org-jira :type git :host github :repo "ahungry/org-jira"))
  (straight-use-package '(org-jira :type git :host github :repo "hansffu/org-jira"))
...
#+end_src
** Tab Bar Format
Display the global mode string in the tab bar
#+begin_src emacs-lisp
...
Set jira url, and exclude the "reporter id" field from the update issue payload.

#+begin_src emacs-lisp
  (add-hook 'org-jira-mode-hook (lambda ()
  				(setq jiralib-url "https://waymark-care.atlassian.net")
  				(add-to-list 'jiralib-update-issue-fields-exclude-list 'reporter)))
#+end_src
* Plantuml Mode
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (setq color-schemes (list
                       "d9ed92-99d98c-34a0a4-7696d0-83678e-c55969-ca6877"
                       "acd322-4ba838-34a0a4-7696d0-83678e-c55969-ca6877"
                       "e8e6a6-bbdca3-5fb5b9-9da5d2-ab7bad-d77984-db858f"
                       "e5e3ab-bad6a6-6aacb0-a4aacd-a287a3-d27f89-d68b94"
                       ))
  (set-face-attribute 'outline-4 nil :slant 'normal)

  (defvar current-color-scheme-index 0
    "Index of the currently active color scheme in `color-schemes`.")

  (defun apply-color-scheme (colors)
    "Apply COLORS (a list of hex codes) to outline faces."
    (dotimes (i (length colors))
      (let ((face (intern (format "outline-%d" (1+ i))))
            (col (nth i colors)))
        (set-face-foreground face col))))

  (defun apply-current-color-scheme ()
    "Apply the color scheme at `current-color-scheme-index`."
    (apply-color-scheme (col-strip (nth current-color-scheme-index color-schemes))))

  (defun cycle-color-scheme ()
    "Advance to the next color scheme in `color-schemes`, cycling back if needed."
    (interactive)
    (setq current-color-scheme-index
          (mod (1+ current-color-scheme-index) (length color-schemes)))
    (apply-current-color-scheme)
    (message "Switched to color scheme %d" (1+ current-color-scheme-index)))
  ...
  (with-eval-after-load 'org
    ...
    (define-key org-mode-map (kbd "C-M-n") 'org-next-visible-heading)
    (define-key org-mode-map (kbd "C-M-p") 'org-previous-visible-heading)
    (define-key org-mode-map (kbd "C-M-u") 'outline-up-heading)
    (define-key org-mode-map (kbd "M-F") 'org-demote-subtree)
    (define-key org-mode-map (kbd "M-B") 'org-promote-subtree)
...
#+begin_src emacs-lisp
  (with-eval-after-load 'mcp-hub
    (setq mcp-hub-servers
          '(
            ("filesystem" . (:command "npx" :args ("-y" "@modelcontextprotocol/server-filesystem" "~/lib"))))))
#+end_src
* vterm
Advise vterm to create buffers with default directory in name, while adding the buffer to the active perspective

#+begin_src emacs-lisp
  (require 'vterm)

  (defun my-vterm-advice (&optional arg)
    "Override `vterm` to include `default-directory` in the buffer name."
    (interactive "P")
    (let* ((directory-name (file-name-nondirectory (directory-file-name default-directory)))
           (name (format "*vterm-%s*" directory-name))
           (buf (vterm--internal #'pop-to-buffer-same-window name)))
      (persp-add-buffer buf)
      (switch-to-buffer buf)
      (hack-dir-local-variables-non-file-buffer)
      buf))

  (advice-add 'vterm :override #'my-vterm-advice)
#+end_src

Increase vterm scrollback.

#+begin_src emacs-lisp
  (setq vterm-max-scrollback 100000)
#+end_src

Keybindings for navigating vterm buffers within the current perspective.

#+begin_src emacs-lisp
    (defun my-persp-vterm-buffers ()
      "Return vterm buffers in the current perspective, in buffer list order."
      (cl-remove-if-not
       (lambda (b) (eq (buffer-local-value 'major-mode b) 'vterm-mode))
       (persp-current-buffers)))

    (defun my-vterm-toggle ()
      "Switch to the most recent vterm in the perspective, or create one.
      If already in a vterm buffer, create a new one instead."
      (interactive)
      (if (eq major-mode 'vterm-mode)
          (call-interactively #'vterm)
        (let ((vterms (my-persp-vterm-buffers)))
          (if vterms
              (switch-to-buffer (car vterms))
            (call-interactively #'vterm)))))

    (defun my-vterm-cycle (direction)
      "Cycle DIRECTION (+1 or -1) through vterm buffers in the current perspective."
      (let* ((vterms (my-persp-vterm-buffers))
             (len (length vterms)))
        (cond
         ((zerop len)
          (message "No vterm buffers in this perspective."))
         ((= len 1)
          (switch-to-buffer (car vterms)))
         (t
          (let* ((idx (cl-position (current-buffer) vterms))
                 (next-idx (if idx
                               (mod (+ idx direction) len)
                             (if (> direction 0) 0 (1- len)))))
            (switch-to-buffer (nth next-idx vterms)))))))

    (defun my-vterm-cycle-next ()
      "Cycle to the next vterm buffer in the current perspective."
      (interactive)
      (my-vterm-cycle 1))

    (defun my-vterm-cycle-prev ()
      "Cycle to the previous vterm buffer in the current perspective."
      (interactive)
      (my-vterm-cycle -1))

    (global-set-key (kbd "C-x C-v") 'my-vterm-toggle)

    (with-eval-after-load 'vterm
      (define-key vterm-mode-map (kbd "C-c C-n") #'my-vterm-cycle-next)
      (define-key vterm-mode-map (kbd "C-c C-p") #'my-vterm-cycle-prev)
      (define-key vterm-mode-map (kbd "M-n") #'vterm-next-prompt)
      (define-key vterm-mode-map (kbd "M-p") #'vterm-previous-prompt))
#+end_src
...
* agent-shell
#+begin_src emacs-lisp
  (defun my-agent-shell-cycle (direction)
    "Cycle DIRECTION (+1 or -1) through agent-shell buffers in the current project."
    (let* ((shells (sort (agent-shell-project-buffers)
                         (lambda (a b) (string< (buffer-name a) (buffer-name b)))))
           (len (length shells)))
      (cond
       ((zerop len)
        (message "No agent-shell buffers in this project."))
       ((= len 1)
        (switch-to-buffer (car shells)))
       (t
        (let* ((idx (cl-position (current-buffer) shells))
               (next-idx (if idx
                             (mod (+ idx direction) len)
                           (if (> direction 0) 0 (1- len)))))
          (switch-to-buffer (nth next-idx shells)))))))

  (defun my-agent-shell-cycle-next ()
    (interactive)
    (my-agent-shell-cycle 1))

  (defun my-agent-shell-cycle-prev ()
    (interactive)
    (my-agent-shell-cycle -1))
  ...
  (with-eval-after-load 'agent-shell
    ...
    (global-set-key (kbd "C-x C-a") 'agent-shell)
    (define-key agent-shell-mode-map (kbd "M-p") #'agent-shell-previous-item)
    (define-key agent-shell-mode-map (kbd "M-n") #'agent-shell-next-item)
    (define-key agent-shell-mode-map (kbd "C-c C-n") #'my-agent-shell-cycle-next)
    (define-key agent-shell-mode-map (kbd "C-c C-p") #'my-agent-shell-cycle-prev))
  ...
  (with-eval-after-load 'compile
    ...
    (let ((compilation-patterns
           '(
             ...
             (elixir-mix "^[[:space:]]*└─[[:space:]]+\\(.*\\.ex\\):\\([0-9]+\\):\\([0-9]+\\)" 1 2 3 1)
             (elixir-mix-error "^\\*\\* .+ on \\(\\(.*\\.ex\\):\\([0-9]+\\):\\([0-9]+\\)\\):" 2 3 4 nil 1))))
...
#+begin_src emacs-lisp
  ...
  (require 'combobulate)
  (define-key combobulate-key-map (kbd "M-n") #'combobulate-navigate-next)
  (define-key combobulate-key-map (kbd "M-p") #'combobulate-navigate-previous)
  (define-key combobulate-key-map (kbd "C-M-n") #'combobulate-navigate-down)
  (define-key combobulate-key-map (kbd "C-M-p") #'combobulate-navigate-up)
#+end_src
** web-mode
...
#+begin_src emacs-lisp
  ...
  (add-to-list 'eglot-server-programs '(elixir-ts-mode "~/.emacs.d/language-servers/elixir/language_server.sh"))
